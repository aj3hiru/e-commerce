...
function rowToProduct(row, categorySlug, subcategorySlug) {
  return {
    ...
    badgeTag: row.badge_tag && row.badge_tag !== "none" ? row.badge_tag : null,
  };
}
...
export async function getProductBySlug(slug) {
  ...
  const product = rowToProduct(row, row.category_slug, row.subcategory_slug);
  const approvedReviews = await sql`
    SELECT customer_name, rating, text FROM product_reviews
    WHERE product_id = ${row.id} AND status = 'approved'
    ORDER BY created_at DESC
  `;
  product.reviews = approvedReviews.map((r) => ({ name: r.customer_name, rating: Number(r.rating), text: r.text }));
  if (approvedReviews.length > 0) {
    product.rating = approvedReviews.reduce((sum, r) => sum + Number(r.rating), 0) / approvedReviews.length;
  }
  return product;
}
