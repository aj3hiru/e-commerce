...
// `fn` receives a tagged-template `sql` function (same call style as getSql())
// bound to that connection, so every statement inside `fn` is part of the
// same transaction. Commits on success, rolls back and rethrows on error.
// Used anywhere multiple writes must succeed or fail together (e.g. POS
// checkout: order + order_items + stock deduction + due/credit record).
export async function withTransaction(fn) {
  ...
  const txSql = async function sql(strings, ...values) {
    let text = strings[0];
    for (let i = 0; i < values.length; i++) {
      text += "?" + strings[i + 1];
    }
  ...
  try {
    ...
    const result = await fn(txSql, conn);
    await conn.commit();
    return result;
  } catch (err) {
    try {
