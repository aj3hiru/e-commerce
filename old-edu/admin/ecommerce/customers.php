...
if (isset($_GET['set_status']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    header("Location: /admin/ecommerce/customers.php" . (isset($_GET['filter']) ? '?filter=' . urlencode($_GET['filter']) : ''));
    exit;
}
...
<style>
.type-pill { display: inline-flex; align-items: center; gap: 0.3rem; font-size: 0.75rem; font-weight: 700; padding: 0.25rem 0.65rem; border-radius: 9999px; }
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item" href="?set_status=active&id=<?= $c['id'] ?><?= $filter ? '&filter=' . $filter : '' ?>">Activate</a></li>
                                            <li><a class="dropdown-item" href="?set_status=inactive&id=<?= $c['id'] ?><?= $filter ? '&filter=' . $filter : '' ?>">Suspend</a></li>
                                        </ul>
                                    </div>
                                </td>
                                <td>
...
$(document).ready(function () {
    $('#admin-table').DataTable({ order: [], columnDefs: [{ orderable: false, targets: [3, 4, 5, 6] }] });
});
